...
import { StyledCard, StyledCardList, StyledContainer, StyledPagination } from "./styles";

const INITIAL_URL = `${API_URL}/pokemon`;

const Home: React.FC = () => {
  const dispatch = useDispatch();
  const homeState = useSelector((state: ApplicationState) => state.modules.home);
  ...
  switch (homeState.pokemons.status) {
    ...
    case "loaded":
      contentRender = homeState.pokemons.results.map((pokemon, index) => {
        return (
          <StyledCard pokemon={pokemon} key={index}/>
        );
      });
      break;
